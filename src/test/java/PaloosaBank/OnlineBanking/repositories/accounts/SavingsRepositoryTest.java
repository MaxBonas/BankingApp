...
public class SavingsRepositoryTest {
    ...
    void setUp() {
        ...
        accountTest1 = new Savings(new Money(BigDecimal.valueOf(9988.77)), accountHolderTest1,
                accountHolderTest2);
        savingsRepository.save(accountTest1);
    }

    @Test
    @DisplayName("Check method findBySecretKey on Savings")
    void findSavingsBySecretKey_OK () {
        String secretKeyTest = accountTest1.getSecretKey();

        assertTrue(savingsRepository.findBySecretKey(secretKeyTest).isPresent());
